...
<div class="search">
    ...
    <Card v-show="currView=='index'">
            ...
            <Form ref="searchForm" :model="searchForm" inline :label-width="0">
                <Form-item ref="searchForm" :model="searchForm" inline :label-width="0" style="display:flex;">
                    <Form-item label="" prop="title">
                        <Input type="text" v-model="searchForm.title" placeholder="请输入科室标题" clearable style="width: 200px" />
                    </Form-item>
        ...
        <Row v-show="openTip"> </Row>
         <Row> 
            <Col v-for="(item, index) in data" :key="index" style="display:inline-block"
             @click.native="selectOneRoom(item.roomId)">
          <Card class="card-main">
            ...
            <div class="tips">
              <Button @click="edit(item)" type="info" size="small" icon="ios-create-outline" ghost>编辑</Button> 
              <Button @click="remove(item)" type="error" size="small" icon="md-trash" ghost>删除</Button> 
            </div>
          </Card>
        </Col>
        </Row> 
...
export default {
    ...
    methods: {
        ...
        getDataList() {
            this.loading = true;
            getHospitalDepartmentList(this.searchForm).then(res => {
                this.loading = false;
                if (res.success) {
